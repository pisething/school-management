...
import static com.piseth.java.school.schoolManagement.property.StudentPropertyFilter.NAME;
...
public class MonthlyScoreServiceImpl implements MonthlyScoreService {
	...
	public List<RankDTO> getRankDTO(Map<String, String> params) {
		...
		Map<Student, List<MonthlyScore>> mapMonthlyScore = getMonthlyScore(params).stream()
				.collect(Collectors.groupingBy(MonthlyScore::getStudent));

		List<Long> studentIds = mapMonthlyScore.keySet().stream().map(Student::getId).toList();

		Map<Long, Student> mapStudent = studentRepository.findAllById(studentIds).stream()
				.collect(Collectors.toMap(Student::getId, Function.identity()));

		List<RankDTO> list = mapMonthlyScore.entrySet().stream().map(entry -> {
			...
			return rankDTO;
		}).sorted((a, b) -> b.getTotalScore().compareTo(a.getTotalScore())).toList();

		List<RankDTO> ranklist = IntStream.range(0, list.size()).mapToObj(index -> {
	...
	private RankDTO toRankDTO(Student student, List<MonthlyScore> mapStudent) {
		...
		Double totalScore = mapStudent.stream().collect(Collectors.summingDouble(m -> m.getScore()));

		Double average = mapStudent.stream().collect(Collectors.averagingDouble(m -> m.getScore()));
